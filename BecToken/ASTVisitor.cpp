...
void visit(ASTNode* node) {
    ...
    if (node->get_node_type() == NodeTypeContractDefinition)
    {
        ...
        string contractDefinitionNodeName = testNode->get_name();
        //cout << "ContractDefinitionNode's Name: " << testNode->get_name() << "\n";
        
        if (contractDefinitionNodeName.compare("PausableToken") == 0) 
	{
            if (count == 0)
               count++;
            else
    ...
    if (node->get_node_type() == NodeTypeFunctionDefinition)
        {
            ...
            cout << "FunctionDefinitionNode's Name: " << fd->get_name() << "\n";
            if (name.compare("batchTransfer") == 0) 
	    {
		if (count1 ==0)
		   count1++;
		else
	    ...
	    if (name.compare("BatchOverflow") == 0) 
	    {
		if (count2 ==0)
		   count2++;
		else
...
void addInitialVariablesToConstructor(FunctionDefinitionNode* fd) 
{
	...
	LiteralNode* literalNode3 = new LiteralNode("2**250");
...
void instrumentBatchTransferFunction(FunctionDefinitionNode* fd)
{
    ...
    shared_ptr<ForStatementNode> forStatement = std::dynamic_pointer_cast<ForStatementNode>(forLoopNode);
    ASTNodePtr a2 = (ASTNodePtr)(forStatement->get_body());
